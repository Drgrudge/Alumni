...
const UserDirectoryPage = () => {
  ...
  const { users, status, error, totalUsers } = useSelector((state) => state.userDirectory);
    ...
    <div>
      ...
      <div className="grid grid-cols-1 mt-15 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {users.map(user => {
          ...
          console.log(`User: ${user.personalDetails.firstName} ${user.personalDetails.lastName}, Profile Picture URL: ${profilePictureUrl}`);
