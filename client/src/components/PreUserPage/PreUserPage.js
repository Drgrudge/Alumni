...
const PreUserPage = () => {
  ...
  const { status, error, admins, alumni, faculty, students } = useSelector(
    (state) => state.preUser
  );
  ...
  const graduationYearOptions = Array.from({ length: 24 }, (v, i) => 2000 + i); // 2000 to 2023
      ...
      <div className="border-2 w-4/5  bg-white my-4 rounded-lg shadow-lg border-gray-500">
        ...
        <div className="mt-4 mx-4 mb-6">
          ...
          <div className="border-t-2 mt-2">
            {selectedType === "admins" && <AdminList data={filteredData()} />}
            {selectedType === "alumni" && <AlumniList data={filteredData()} />}
            {selectedType === "faculty" && <FacultyList data={filteredData()} />}
            {selectedType === "students" && <StudentList data={filteredData()} />}
          </div>
        </div>
      </div>
    </div>
